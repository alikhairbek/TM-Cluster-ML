...
# + d-band Center + Nano-Alloys Virtual Screening
# ==========================================================
...
print("Engineering features are being calculated....")
...
print("\n=== Magical numbers extracted from Global Minimum ===")
for m in metals:
    data = magic_data[m]
...
def generate_alloy_xyz(metal1, metal2, ratio, n_atoms=38):
    """ XYZ """
    output_folder = f"{ROOT}/Predicted_Nano_Alloys"
    ...
    with open(xyz_path, "w") as f:
        f.write(f"{n_atoms}\n")
        f.write(f"{alloy_name} alloy cluster generated by Imperial ML Suite\n")
...
# =========================================
print("\n=== 25 figures are being generated (high Leve) ===")
...
ax.set_title("Stability vs Cluster Size (Global Minimum)")
...
# avg_cn و dband_center
def compute_avg_cn(coords, cutoff_factor=1.35):
    if len(coords) < 2: return np.nan
    dist_matrix = squareform(pdist(coords))
    np.fill_diagonal(dist_matrix, np.inf)
    min_dists = np.min(dist_matrix, axis=1)
    est_bond = np.mean(min_dists)
    cutoff = cutoff_factor * est_bond
    cn = np.sum(dist_matrix < cutoff, axis=1).astype(float)
    return float(np.mean(cn))

df["avg_cn"] = df["coords"].apply(compute_avg_cn)

def calculate_dband_center(metal, n_atoms, avg_cn):
    dband_ref = {"Fe":-0.92, "Co":-1.17, "Ni":-1.29, "Ru":-1.41, "Rh":-1.73,
                 "Pd":-1.83, "Os":-2.11, "Ir":-2.25, "Pt":-2.25}
    size_correction = -0.35 * np.exp(-n_atoms / 50)
    cn_correction = -0.08 * (avg_cn - 6.0)
    return dband_ref.get(metal, -1.5) + size_correction + cn_correction

df["dband_center"] = df.apply(lambda row: calculate_dband_center(row["metal"], row["n_atoms"], row.get("avg_cn", 6.0)), axis=1)

# Phase Transitions
print("\n Discovering structural transition zones")
df["dimensionality_index"] = df["asphericity"] * (1 - df["compactness"])
phase_transitions = {}
for m in metals:
    sub = df[df["metal"] == m].copy()
    if len(sub) < 10: continue
    X_geo = sub[["asphericity", "compactness", "n_atoms"]].values
...
ax.set_xlabel("Cluster Size (n)")
ax.set_ylabel("Asphericity (Shape Deviation)")
...
# Figure 24: d-band Center
fig, ax = plt.subplots()
for m in metals:
    sub = df[df["metal"] == m]
    ax.scatter(sub["dband_center"], sub["binding_energy_per_atom"], s=25, alpha=0.8, color=colors.get(m), label=m)
ax.set_xlabel("d-band Center (eV)")
ax.set_ylabel("Binding Energy per Atom (eV)")
ax.set_title("d-band Center vs Stability & Catalytic Activity")
ax.legend()
save_fig(fig, 24, "d_band_Center_vs_Stability")

# Figure 25: Virtual Nano-Alloys +  XYZ
print("\n Virtual Screening for Nano-Alloys +  XYZ")
alloy_candidates = []
for base in ["Fe", "Co", "Ni"]:
    for alloy_m in ["Pd", "Pt", "Ru"]:
        for ratio in [0.3, 0.5, 0.7]:
            ...
            feat[feature_cols.index("compactness")] = 0.78
            feat[feature_cols.index("asphericity")] = 0.22
            feat[feature_cols.index("radius_gyration")] = 4.5
...
alloy_df.to_excel(f"{ROOT}/Tables/Virtual_Nano_Alloys.xlsx", index=False)
...
ax.set_title("Virtual Screening of Nano-Alloys")
save_fig(fig, 25, "Virtual_Nano_Alloys_Screening")
...
doc.add_heading('Phase Transitions, Electronic-Geometric Coupling, d-band Center, and Nano-Alloy Design (N ≤ 55)', level=1)
...
doc.add_paragraph("This analysis combines supervised ML with unsupervised phase detection and virtual screening "
                  "to uncover new chemical insights in transition metal nanoclusters.", style='Intense Quote')

# Table 1: Model Performance with Cross-Validation
doc.add_heading('Table 1: Performance of the 7 Machine Learning Models (5-Fold CV)', level=2)
...
# Table 2: Magic Numbers (Global Minimum DFT)
doc.add_heading('Table 2: Magic Numbers Extracted from Global Minimum DFT Energies (Δ²E)', level=2)
...
# Table 3: Phase Transitions Detected
...
# Table 4: ML vs DFT Comparison (by cluster size)
doc.add_heading('Table 4: ML Predictions vs DFT Values by Cluster Size', level=2)
comparison = df_test.groupby("n_atoms").agg({
    "Actual": "mean",
    "Predicted": "mean",
    "Error": "mean"
}).round(5).reset_index()
comparison.rename(columns={"Actual":"Actual_DFT", "Predicted":"Predicted_ML"}, inplace=True)
...
for _, row in comparison.iterrows():
    ...
    r[0].text = str(row["n_atoms"])
    r[1].text = f"{row['Actual_DFT']:.4f}"
    r[2].text = f"{row['Predicted_ML']:.4f}"
    r[3].text = f"{row['Error']:.4f}"

# Table 5: d-band Center Statistics
doc.add_heading('Table 5: d-band Center Analysis (Catalytic Relevance)', level=2)
db_stats = df.groupby("metal").agg({
    "dband_center": ["mean", "min", "max"],
    "binding_energy_per_atom": "mean"
}).round(3)
db_stats.columns = ["Avg_dband", "Min_dband", "Max_dband", "Avg_BE"]
db_stats = db_stats.reset_index()

table5 = doc.add_table(rows=1, cols=5)
hdr = table5.rows[0].cells
for i, col in enumerate(["Metal", "Avg d-band (eV)", "Min d-band", "Max d-band", "Avg BE (eV)"]):
    hdr[i].text = col
for _, row in db_stats.iterrows():
    r = table5.add_row().cells
    r[0].text = row["metal"]
    r[1].text = f"{row['Avg_dband']:.3f}"
    r[2].text = f"{row['Min_dband']:.3f}"
    r[3].text = f"{row['Max_dband']:.3f}"
    r[4].text = f"{row['Avg_BE']:.3f}"

# Table 6: Top 10 Most Stable Clusters (Global Min)
doc.add_heading('Table 6: Top 10 Most Stable Clusters (Lowest Binding Energy)', level=2)
top_stable = df.loc[df.groupby(["metal", "n_atoms"])["binding_energy_per_atom"].idxmin()]
top_stable = top_stable.sort_values("binding_energy_per_atom").head(10)[["metal", "n_atoms", "binding_energy_per_atom", "magnetic_moment", "avg_cn"]].round(4)
...
for _, row in top_stable.iterrows():
    ...
    r[4].text = f"{row['avg_cn']:.2f}"

# Table 7: Virtual Nano-Alloys Screening (Top 10)
doc.add_heading('Table 7: Virtual Screening of Nano-Alloys (Most Promising Candidates)', level=2)
...
# Table 8: Summary of Key Insights
doc.add_heading('Table 8: Summary of Key Scientific Insights', level=2)
insights = [
    ["Insight", "Description"],
    ["Phase Transitions", f"Detected 2D→3D transitions at specific sizes (e.g., Pt at N≈{phase_transitions.get('Pt', 'N/A')})"],
    ["Magnetic-Geometric Coupling", "Strong correlation between magnetic moment and binding energy"],
    ["Electronic-Geometric Coupling", "PDP shows non-linear interaction between compactness and magnetic moment"],
    ["Catalytic Potential", "d-band center analysis identifies promising sizes for CO/H₂ adsorption"],
    ["Nano-Alloy Candidates", f"Top stable alloys discovered via transfer learning (e.g., {top_alloys.iloc[0]['Alloy']})"]
]

table8 = doc.add_table(rows=len(insights), cols=2)
